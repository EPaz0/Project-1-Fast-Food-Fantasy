...
int main(int argc, char *argv[])
{
    ...
    query.exec("drop table if exists menu");
    if(!query.isActive())
        qWarning() << "ERROR: drop table menu" << query.lastError().text();

    query.exec("drop table if exists distance");
    if(!query.isActive())
        qWarning() << "ERROR: drop table distance" << query.lastError().text();


    query.exec("CREATE TABLE IF NOT EXISTS restaurantList (id INTEGER PRIMARY KEY, restaurantName TEXT, distanceToSaddleback FLOAT)");
    if(!query.isActive())
    ...
    if(!query.exec("CREATE TABLE distances ('fromRestaurant' INTEGER, 'toRestaurant' INTEGER, 'distance' FLOAT, UNIQUE(fromRestaurant, toRestaurant))"))
